...
static const char vernum[] = "v62.S3.01";
char devname[30];
#define STOP_START_GPIO 46
...
static esp_err_t init_sdcard()
{
  ...
  //int succ = SD_MMC.begin("/sdcard", true, true, SDMMC_FREQ_DEFAULT,5 );
  ...
  if (succ) {
    ...
    Serial.println("Check sd card gpios 38,39,40, not grounded, or grounded with 10k resistors!\n\n");
    major_fail();
  }
...
<h1>%s<br>ESP32-CAM Video Recorder Junior %s <br><font color="red">%s</font></h1><br>
 ...
 <h3>Recording is <font color="red"> %s </font> - overrides hardware pin 46 stop/start</h3>
...
James Zahary - Jun 18, 2026 -- May 18, 2022<br>
...
void setup() {
  ...
  pinMode(STOP_START_GPIO, INPUT_PULLUP);        // pull this down to stop recording
  //pinMode(47, INPUT_PULLUP);        // pull this down switch wifi
...
void the_camera_loop (void* pvParameter) {
  ...
  frame_cnt = 0;
  start_record_2nd_opinion = digitalRead(STOP_START_GPIO);
  start_record_1st_opinion = digitalRead(STOP_START_GPIO);
  start_record = 0;
  ...
  while (1) {
    ...
    if ( (frame_cnt == 0 && start_record == 0)) {
      ...
      if (we_are_already_stopped == 0) jpr("\n\nDisconnect STOP_START_GPIO 46 from GND to start recording or http://192.168.1.100/start \n\n");

...
int  framesize = 7;                //  10 UXGA, 9 SXGA, 7 SVGA, 6 VGA, 5 CIF // will be superceded with pin 13
int  quality = 12;                 //  quality on the 1..63 scale  - lower is better quality and bigger files - must be higher than the jpeg_quality in camera_config
...
const int avi_header[AVIOFFSET] PROGMEM = {
  ...
  0x76, 0x30, 0x39, 0x20, 0x4C, 0x49, 0x53, 0x54, 0x00, 0x01, 0x0E, 0x00, 0x6D, 0x6F, 0x76, 0x69,
};
...
void loop() {
  ...
  stop_1st_opinion = digitalRead(12) + digitalRead(12) + digitalRead(12);
  ...
  if (frame_cnt == 1 && stop == 0) {
    ...
    new_frame = digitalRead(13);

    if (new_frame == 0) {
      if (framesize == 7) {
        Serial.println("Switching to UXGA");
        framesize = 10;
        sensor_t * ss = esp_camera_sensor_get();
        ss->set_quality(ss, quality);
        ss->set_framesize(ss, (framesize_t) 10);
      ...
      if (framesize == 10) {
        Serial.println("Switching to SVGA");
        framesize = 7;
        sensor_t * ss = esp_camera_sensor_get();
        ss->set_quality(ss, quality);
        ss->set_framesize(ss, (framesize_t)7);
    ...
    if (frame_cnt % 100 == 0 ) {     // print some status every 100 frames
      if (frame_cnt == 100) {
      ...
      Serial.printf("So far: %04d frames, in %06d ms, for last 100 frames: avg frame size %06d, %.2f fps ...\n", frame_cnt, millis() - avi_start_time, most_recent_avg_framesize, most_recent_fps);
      total_delay = 0;
